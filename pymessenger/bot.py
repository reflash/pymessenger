...
class Bot:
	...
	def send_attachment_url(self, recipient_id, attachment_type, attachment_url, quick_replies=None):
	...
	def send_attachment_id(self, recipient_id, attachment_type, attachment_id, quick_replies=None):
		"""Send an attachment to the specified recipient using URL.
		Input:
			recipient_id: recipient id to send to
			attachment_type: type of attachment (image, video, audio, file)
	...
	def send_text_message(self, recipient_id, message, quick_replies=None):
	...
	def send_audio_url(self, recipient_id, audio_url, quick_replies=None):
		...
		if quick_replies is None:
			...
			return self.send_attachment_url(recipient_id, "audio", audio_url, quick_replies)

	def send_audio_id(self, recipient_id, attachment_id, quick_replies=None):
		"""Send audio to specified recipient using URL.
		Audio must be MP3 or WAV
		https://developers.facebook.com/docs/messenger-platform/send-api-reference/audio-attachment
		Input:
		...
		"""
		if quick_replies is None:
			return self.send_attachment_id(recipient_id, "audio", attachment_id)
		else:
			return self.send_attachment_id(recipient_id, "audio", attachment_id, quick_replies)
